...
final class BehaviorDispenseArmor extends BehaviorDefaultDispenseItem {
    ...
    protected ItemStack dispenseStack(IBlockSource par1IBlockSource,
        ...
        EnumFacing enumfacing = BlockDispenser.getFacing(par1IBlockSource
                .getBlockMetadata());
        int i = par1IBlockSource.getXInt() + enumfacing.getFrontOffsetX();
        int j = par1IBlockSource.getYInt() + enumfacing.getFrontOffsetY();
        int k = par1IBlockSource.getZInt() + enumfacing.getFrontOffsetZ();
        AxisAlignedBB axisalignedbb = AxisAlignedBB.getAABBPool().getAABB(i, j,
                k, i + 1, j + 1, k + 1);
        List<?> list = par1IBlockSource.getWorld().selectEntitiesWithinAABB(
                EntityLiving.class, axisalignedbb,
        ...
        if (list.size() > 0) {
            ...
            entityliving.setCurrentItemOrArmor(i1, itemstack1); // Forge:
                                                                // Vanilla
                                                                // bug
                                                                // fix
                                                                // associated
