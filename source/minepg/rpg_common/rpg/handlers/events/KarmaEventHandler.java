...
public class KarmaEventHandler {
    ...
    private static final int[] bonemealHandleIds = new int[] {
            Block.sapling.blockID, Block.mushroomBrown.blockID,
            Block.mushroomRed.blockID, Block.melonStem.blockID,
            Block.pumpkinStem.blockID, Block.cocoaPlant.blockID,
            Block.grass.blockID };
    ...
    public static void onAfterBlockSet() {
        if (playerPlacingBlock != null) {
            ...
            CountableKarmaEvent event = placingIronGolem ? CountableKarmaEvent.CREATE_IRONGOLEM
                    : CountableKarmaEvent.CREATE_SNOWGOLEM;
    ...
    // is modified
    public static void onBeforePlayerPlaceBlock(Item item, World world, int x,
        ...
        if (!world.isRemote && item.itemID == Block.pumpkin.blockID) {
            ...
            boolean isIronGolem = isSnowGolem ? false
                    : (world.getBlockId(x, y - 1, z) == Block.blockIron.blockID
                            ...
                            && (world.getBlockId(x - 1, y - 1, z) == Block.blockIron.blockID && world
                                    .getBlockId(x + 1, y - 1, z) == Block.blockIron.blockID) || (world
                            .getBlockId(x, y - 1, z - 1) == Block.blockIron.blockID && world
    ...
    public static void onItemPotionUse(ItemStack potion, EntityPlayer player) {
        if (!player.worldObj.isRemote
                && ItemPotion.isSplash(potion.getItemDamage())) {
    ...
    // CrymodTransformer
    public static void onMobSpawnerBreak(EntityPlayer player, World world,
            int x, int y, int z) {
        if (!world.isRemote) {
    ...
    public void onBonemealUse(BonemealEvent evt) {
        if (evt.getResult() == Result.ALLOW
                || Ints.contains(bonemealHandleIds, evt.ID)
                || (evt.ID > 0 && Block.blocksList[evt.ID] instanceof BlockCrops)) {
    ...
    public void onEntityAttack(LivingAttackEvent evt) {
        if (evt.source instanceof EntityDamageSource
                && ((EntityDamageSource) evt.source).getEntity() instanceof EntityPlayerMP
                && evt.entity instanceof EntityPigZombie) {
            PlayerInformation info = forPlayer(((EntityDamageSource) evt.source)
                    .getEntity());
    ...
    public void onEntityDeath(LivingDeathEvent evt) {
        if (evt.source.getEntity() instanceof EntityPlayerMP) {
            ...
            } else if (evt.entity instanceof EntityIronGolem
                    || evt.entity instanceof EntitySnowman) {
    ...
    public void onEntityInteract(EntityInteractEvent evt) {
        if (evt.entityPlayer instanceof EntityPlayerMP) {
            if (evt.target instanceof EntityAnimal) {
                ...
                if (animal.isBreedingItem(evt.entityPlayer
                        .getCurrentEquippedItem())
                        && animal.getGrowingAge() == 0) {
                    MinePGUtil.getEntityData(animal).setString("breedingOwner",
                            evt.entityPlayer.username);
                }
            } else if (evt.target instanceof EntityZombie) {
                EntityZombie zombie = (EntityZombie) evt.target;
                ItemStack currentItem = evt.entityPlayer
                        .getCurrentEquippedItem();
                if (currentItem != null
                        && currentItem.getItem() == Item.appleGold
                        && currentItem.getItemDamage() == 0
                        && zombie.isVillager()
                        && zombie.isPotionActive(Potion.weakness)) {
                    MinePGUtil.getEntityData(zombie).setString("cureOwner",
                            evt.entityPlayer.username);
                }
            }
        }
